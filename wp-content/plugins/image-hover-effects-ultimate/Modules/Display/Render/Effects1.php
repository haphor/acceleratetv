        ...
        if ('yes' == $style['display_post_load_more']) {
            if ($style['display_post_load_more_type'] == 'button'):
                echo '  <div class="oxi-image-hover-load-more-button-wrap oxi-bt-col-sm-12">
                            <button class="oxi-image-load-more-button" data-class="OXI_IMAGE_HOVER_PLUGINS\Modules\Display\Files\Style_1_Post_Query" data-function="__ajax_template" data-args=\'' . json_encode($args) . '\' data-settings=\'' . json_encode($settings) . '\' data-page="1">
                                    <div class="oxi-image-hover-loader button__loader"></div>
                                    <span>' . $style['display_post_load_button_text'] . '</span>
                            </button>
                        </div>';
            else:
                echo '<div class="oxi-image-hover-load-more-infinite" data-class="OXI_IMAGE_HOVER_PLUGINS\Modules\Display\Files\Style_1_Post_Query" data-function="__ajax_template" data-args=\'' . json_encode($args) . '\' data-settings=\'' . json_encode($settings) . '\' data-page="1">
                      </div>';
            endif;
        }
    }
