...
<?php }
    ...
    $order_sub_categories = get_term_meta($current_term->term_id, 'wpvs_sub_cat_order', true);
    $child_tax_filter = array(
        'parent' => $current_term->term_id,
    );
    if( ! empty($order_sub_categories) ) {
        $child_tax_filter['orderby'] = $order_sub_categories;
    }
    $children_taxomonies = get_terms(
        'rvs_video_category',
        $child_tax_filter
    );
    ...
    if($contains_shows) :
        if( empty($wpvs_browsing_layout) || $wpvs_browsing_layout == 'grid') {
            ...
            include(locate_template('template/browse-show-sliders.php', false , false));
        }
    else :
        ...
        else : get_template_part('nothing-found'); endif; endif; ?>
