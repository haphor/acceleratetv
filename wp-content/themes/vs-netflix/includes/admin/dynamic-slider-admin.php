...
function wpvs_slider_admin_scripts() {
    ...
    wp_register_style( 'wpvs-featured-area-editor-css', $wpvs_theme_directory . '/css/admin/slide-editor.css', '',  $vs_netflix_current_version);
    wp_register_script( 'wpvs-featured-area-blocks-js', $wpvs_theme_directory . '/js/admin/homeblocks.js', array('jquery', 'jquery-ui-sortable'), $vs_netflix_current_version, true);
    wp_register_script( 'wpvs-block-image-uploader', $wpvs_theme_directory . '/js/admin/slideloader.js', array('jquery'), $vs_netflix_current_version, true);
...
function wpvs_dynamic_sliders() {
	if ( !current_user_can( 'manage_options' ) )  {
		wp_die( __( 'You do not have sufficient permissions to access this page.', 'vs-netflix' ) );
	}
    ...
    $video_categories = get_terms(array('taxonomy' => 'rvs_video_category', 'hide_empty' => false, 'parent' => 0));
	?><div class="wrap">
		
		<h2><?php _e('Featured Area Sliders', 'vs-netflix'); ?></h2>
                ...
                ?>
                    <div class="rogueDynamicSlider rvs-border-box" id="section_<?php echo $slider['id']; ?>" >
						...
						<div class="sliderControls">
							...
							<label class="btn saveSlideButton saveTitle"><?php _e('Save Title', 'vs-netflix'); ?></label>
							
							<button class="btn deleteSection"><?php _e('Delete Slider', 'vs-netflix'); ?></button>
							
							<button class="btn addNewBlock"><?php _e('New Slide', 'vs-netflix'); ?></button>
                        ...
                        <div class="wpvs-slider-settings rvs-border-box">
                            ...
                            <label class="btn save-slider-settings"><?php _e('Save Settings', 'vs-netflix'); ?></label>
                        </div>
						...
						<?php if( isset($slider['blocks']) && ! empty($slider['blocks'])) {
                            ...
                            <div class="homeBlockEdit" id="<?php echo $block['id']; ?>">
                                ...
                                <div class="wpvs-edit-slide-detail">
                                    ...
                                    <label class="upload_button btn wpvs-media-button wpvs-blue-button"><span class="dashicons dashicons-format-image"></span> <?php _e('Set Image', 'vs-netflix'); ?></label>
                                    <label class="wpvs-set-video btn wpvs-media-button wpvs-green-button"><span class="dashicons dashicons-video-alt3"></span> <?php _e('Set Video', 'vs-netflix'); ?></label>
                                </div>
                                ...
                                <div class="wpvs-select-background-video">
                                    ...
                                    <div class="wpvs-edit-slide-detail">
                                        <label><?php _e('YouTube URL', 'vs-netflix'); ?>:</label>
                                    ...
                                    <div class="wpvs-edit-slide-detail">
                                        <label><?php _e('Vimeo URL', 'vs-netflix'); ?>:</label>
                                    ...
                                    <div class="wpvs-edit-slide-detail">
                                        <label><?php _e('HTML iFrame', 'vs-netflix'); ?>:</label>
                                    ...
                                    <div class="wpvs-edit-slide-detail">
                                        <label><?php _e('Upload', 'vs-netflix'); ?>:</label>
                                        <input type="button" class="button button-primary set-slide-wordpress-video" value="<?php _e('Choose Video', 'vs-netflix'); ?>" />
                                        <input type="hidden" class="slide-video-url" value="<?php echo $block_video_url; ?>" hidden/>
                                    </div>
                                    <div class="wpvs-edit-slide-detail">
                                        <label><?php _e('Mute Video', 'vs-netflix'); ?>:</label>
                                        <input type="checkbox" class="set-slide-muted-video" value="1" <?php checked(1, $block_video_muted); ?> />
                                    </div>
                                    <div class="wpvs-edit-slide-detail">
                                        <label><?php _e('Aspect Ratio', 'vs-netflix'); ?>:</label>
                                        <select class="set-video-aspect-ratio" name="video-aspect-ratio" >
                                           <option value="" <?php selected('', $slide_aspect_ratio); ?>><?php _e('16:9 (Default)', 'vs-netflix'); ?></option>
                                           <option value="cinematic" <?php selected('cinematic', $slide_aspect_ratio); ?>><?php _e('2.39:1 (Cinematic)', 'vs-netflix'); ?></option>
                                        </select>
                                    </div>
                                </div>
                                
                                <div class="wpvs-edit-slide-detail">
                                    <label><?php _e('Videos', 'vs-netflix'); ?>:</label>
                                ...
                                <div class="wpvs-edit-slide-detail">
									<select class="selectPage video-category-dropdown" name="video-category-dropdown" >
									   <option value="/"><?php _e('Video Categories', 'vs-netflix'); ?></option>
                                ...
                                <div class="wpvs-edit-slide-detail">
									<select class="selectPage page-dropdown" name="page-dropdown">
                                        <option value="/"><?php _e('Pages', 'vs-netflix'); ?></option>
                                ...
                                <?php } if( ! empty($categories) ) { ?>
									<div class="wpvs-edit-slide-detail">
                                        <select class="selectPage blog-dropdown" name="blog-dropdown" >
                                           <option value="/"><?php _e('Categories', 'vs-netflix'); ?></option>
                                ...
                                <div class="wpvs-edit-slide-detail">
									<select class="selectPage post-dropdown" name="post-dropdown" >
									   <option value="/"><?php _e('Posts', 'vs-netflix'); ?></option>
                                ...
                                <div class="wpvs-edit-slide-detail">
                                    <label><?php _e('Image Alt', 'vs-netflix'); ?>:</label>
                                ...
                                ?>
                                <div class="wpvs-edit-slide-detail">
                                    <label><?php _e('Button Size', 'vs-netflix'); ?>:</label>
									<select class="wpvs-slide-button-size post-dropdown" name="wpvs-slide-button-size">
								        <option value="default" <?php selected('default', $block['button_size']); ?>><?php _e('Normal', 'vs-netflix'); ?></option>
								        <option value="large" <?php selected('large', $block['button_size']); ?>><?php _e('Large', 'vs-netflix'); ?></option>
                                    </select>
                                </div>

                                <div class="wpvs-edit-slide-detail">
								    <label><?php _e('Links To', 'vs-netflix'); ?>:</label>
                                ...
                                ?>
                                <div class="wpvs-edit-slide-detail">
                                    <label><?php _e('Align Content', 'vs-netflix'); ?>:</label>
									<select class="select-alignment post-dropdown" name="align-content">
								        <option value="left" <?php selected('left', $block['align']); ?>><?php _e('Left', 'vs-netflix'); ?></option>
								        <option value="center" <?php selected('center', $block['align']); ?>><?php _e('Center', 'vs-netflix'); ?></option>
                                        <option value="right" <?php selected('right', $block['align']); ?>><?php _e('Right', 'vs-netflix'); ?></option>
                                    </select>
                                </div>
                                <div class="openLink">
									<label><?php _e('Link entire slide', 'vs-netflix'); ?></label>
									<?php 
								...
								<div class="openLink">
									<label><?php _e('Open in new tab', 'vs-netflix'); ?></label>
									<?php 
                                        if( ! isset($block['tab']) ) {
                                            $block['tab'] = 0;
                                        }
								...
								<label class="rogue-edit-slider"><span class="dashicons dashicons-edit"></span> <?php _e('Edit', 'vs-netflix'); ?></label>
					...
					<hr>
				<?php } } ?><button id="addNewSection" class="btn newSection"><?php _e('Add New Slider', 'vs-netflix'); ?></button>
