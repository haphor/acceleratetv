...
function wpvs_video_comment_review_fields() {
    ...
    if( $post && $post->post_type == 'rvs_video' ) {
        $wpvs_video_review_fields = '<div class="wpvs-rating-container"><label for="rating">'. __('Rating', 'vs-netflix') . '</label><div class="wpvs-rating-box">';
        for( $i=1; $i <= 5; $i++ ) {
            $wpvs_video_review_fields .= '<span class="ease3 dashicons dashicons-star-empty wpvs-video-rating-star" data-rating="'. $i .'"></span>';
        }
        $wpvs_video_review_fields .= '<label id="wpvs-missing-rating" class="wpvs-review-error">'.__('Please select a rating', 'vs-netflix').'</label>';
